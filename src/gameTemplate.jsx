...
function gameTemplate({ onComplete, piecesArray, chanceLength, reset }) {
      ...
      setTimeout(() => {
        if (flippedPieces[0].emoji === flippedPieces[1].emoji) {
          const solveUpdate = shuffledPieces.map((data) => {
            if (
              data.position === flippedPieces[0].position ||
  ...
  const winningMSg = () => {
    const check =
      shuffledPieces.length > 0 && shuffledPieces.every((data) => data.solved);
    if (check && !wonGame) {
      setWonGame(true);
      // alert("YOU WONN!!");
  ...
  return (
    <main>
      <h3>
        Memory Game - Level {levelCount} Chances left {tryCount}
      </h3>
      {tryCount > 0 && levelCount <= 4 && (
        <div className="conteiner">
          {shuffledPieces.map((data, index) => {
            return (
              <div className="parent-box">
                <div
                  className={`box-back ${data.active ? "flipped" : ""}`}
                  key={index}
      ...
      {wonGame && (
        <div>
          {levelCount < 4 && (
            <div>
              <h1>Level {levelCount} Fineshed!</h1>
              <button
                className="finished"
                onClick={() => {
